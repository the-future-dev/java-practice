...
public class MyQualityPane extends BorderPane {
	private final NumberFormat fP = NumberFormat.getPercentInstance();
...
//		mostrato sotto - che non cambia nel tempo.
		...
		percentageTable.setText(controller.printTabellaPercentuali());
		...
		graph.setTitle("% prodotti entro qualità standard");
		
		
		
		XYChart.Series<String,Number> products = new XYChart.Series<>();
//		products.setName("PRODOTTI");
		for (String s : controller.getTabellaPercentuali().keySet()) {
			XYChart.Data<String, Number> w = new XYChart.Data<>(s, controller.getTabellaPercentuali().get(s));
			products.getData().add(w);
		}
		graph.getData().add(products);
//		setLegendVisible
//		
		this.setTop(top);
	...
	public void pickedADescrizione(ActionEvent e) {
		String str = "Dettaglio"+combo.getValue()+"\n";
		str+= "# misure:"+controller.getMisure(combo.getValue()).size()+"\n";
		str+= "# prodotti entro il range: "+controller.getListaProdottiEntroRange(combo.getValue()).size()+"\n";
		str+= "% proddotti entro il range: "+fP.format(controller.getPercentualeProdottiEntroRange(combo.getValue()));
		dettaglio.setText(str);
