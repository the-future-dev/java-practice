...
public class SudokuBoard {
	...
	public void setCellRow(int row, int [] colNumbers) {
		...
		for (int i = 0; i<DIM; i++) {
			if (colNumbers[i]<0 || colNumbers[i] >9)
...
//		check
	...
	public boolean setCell(int row, int col, int digit) {
		if (!(row>= 0 && row <DIM) || !(row>= 0 && row <DIM) || !(digit>= 0 && digit<=DIM))
	...
	public String toString() {
		String str = "Official Board: \n";
		for (int i = 0; i< DIM; i++) {
			for (int j = 0; j<DIM; j++) {
				if (board[i][j].isEmpty()) {
					str+=" ";
				} else {
					str+=board[i][j].getAsInt();
				}
				str+="\t";
			}
			str+="\n";
