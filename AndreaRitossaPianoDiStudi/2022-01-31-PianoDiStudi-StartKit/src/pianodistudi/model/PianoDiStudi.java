	...
	private boolean verificaSettoriAttivita(Ordinamento ord) {
		...
		for (Tipologia type : lt) {
			...
			for (AttivitaFormativa activity : activityList) {
				boolean found = false;
				for (Ssd sd : possibleSsd) {
					if (sd.equals(activity.getSsd())) {
						found = true;
					}
				}
				if (possibleSsd.contains(Ssd.QUALSIASI)) {
					found = true;
				}
				if (!found) {
					logger.append(activity.getSsd() +" status = false\n");
					System.out.println(logVerifica());
					return false;
				} else {
					logger.append(activity.getSsd()+" status = true\n");
				}
			}
		}
		return true;
