...
public class MainPane extends BorderPane {
	...
	private void myHandler(ActionEvent ev) {
		...
		AttivitaFormativa activity = controller.getAttivitaFormativaPerNome(afCombo.getValue());
		Tipologia type = tipCombo.getValue();
		if (activity == null || type == null)
			Controller.alert("ERROR", "nullable input", "");
		
		try {
			if (ev.getSource().equals(inserisci)) {
				controller.inserisci(activity, type);
			} else {
//				rimuovi
				controller.rimuovi(activity, type);
			}
	...
	private void checkOrd(ActionEvent ev) {
		...
		if (controller.rispettaOrdinamento())
			outputVerifica.setText("Il piano soddisfa l'ordinamento");
		else
			outputVerifica.setText(controller.getLogVerificaOrdinamento());
	}
