...
class SsdTest {
	...
	@Test
	void testOK_sigleExtra() {
		assertEquals("qualsiasi", Ssd.QUALSIASI.getValue());
		assertEquals("prova finale", Ssd.PROVAFINALE.getValue());
		assertEquals("lingua straniera", Ssd.LINGUASTRANIERA.getValue());
	}
	...
	void testOK_of() {
		...
		assertEquals(Ssd.INGINF05, Ssd.of("ING-Inf/05"));
		assertEquals(Ssd.INGINF05, Ssd.of("ING-iNF/05"));
		// gli altri li controlliamo solo in un'unica versione, tanto maiuscole/minuscole sono già testate sopra
		...
		assertEquals(Ssd.IUS14, Ssd.of("Ius/14"));
	}
