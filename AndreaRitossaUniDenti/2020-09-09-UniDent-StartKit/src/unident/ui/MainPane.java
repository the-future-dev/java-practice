...
public class MainPane extends BorderPane {
	...
	private void addExam() {
		...
		LocalDate data = picker.getValue();
		Voto v = votoCombo.getValue();
		
		if (af == null || data == null || v == null)
			UniDentApp.alert("NOT SELECTED", "dammi le info giuste bastardo", "inserisci l'arrivita formativa, il voto e la data corretti");
		else {
			try {
				controller.aggiornaCarriera(new Esame(af, v, data));
			}catch (IllegalArgumentException e) {
				UniDentApp.alert("Errore in picking", "non hai inserito bene", e.getLocalizedMessage());
			}
		}
