...
//		----
		BillingFrequency bf = (radioAnnual.isSelected())? BillingFrequency.ANNUAL : BillingFrequency.MONTHLY;
		Optional<Month> oM = (bf.equals(BillingFrequency.ANNUAL))? Optional.empty() : Optional.of(pickerMonth.getValue().getMonth());
		Bill b = new Bill(bf, cost, cons, fCost, vCost, cm3, em3, oM);
		controller.computeShare(b);
		double diff = controller.getDiffCons(b);
		
		output.setText("la differenza tra il consumo presunto e quello effettivo è: "+diff);
	} 
